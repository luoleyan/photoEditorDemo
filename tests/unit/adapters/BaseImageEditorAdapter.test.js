...
describe("BaseImageEditorAdapter", () => {
  ...
  describe("图像操作", () => {
    ...
    test("应该正确裁剪图像", async () => {
      ...
      expect(adapter.mockCrop).toEqual({
        x: 10,
        y: 20,
        width: 100,
        height: 150,
  ...
  describe("错误处理", () => {
    test("未初始化时操作应该抛出错误", async () => {
      await expect(adapter.loadImage("test.jpg")).rejects.toThrow(
        "Adapter is not initialized"
      );
      await expect(adapter.resize(100, 100)).rejects.toThrow(
        "Adapter is not initialized"
      );
      await expect(adapter.crop(0, 0, 50, 50)).rejects.toThrow(
        "Adapter is not initialized"
      );
    });
  });
