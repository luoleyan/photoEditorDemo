...
describe("StateManager", () => {
  ...
  describe("状态更新", () => {
    ...
    test("应该正确更新现有状态", () => {
      ...
      const newStateId = stateManager.updateState(
        updates,
        "update",
        "Updated image"
      );
  ...
  describe("状态限制", () => {
    test("应该正确限制状态数量", () => {
      ...
      // 创建超过限制的状态
      const stateId1 = limitedStateManager.createState(
        "fabric",
        null,
        "State 1"
      );
      const stateId2 = limitedStateManager.createState(
        "konva",
        null,
        "State 2"
      );
      const stateId3 = limitedStateManager.createState("tui", null, "State 3");
      const stateId4 = limitedStateManager.createState(
        "fabric",
        null,
        "State 4"
      );
  ...
  describe("状态比较", () => {
    ...
    beforeEach(() => {
      stateId1 = stateManager.createState(
        "fabric",
        { src: "test1.jpg" },
        "State 1"
      );
      stateId2 = stateManager.createState(
        "fabric",
        { src: "test2.jpg" },
        "State 2"
      );
  ...
  describe("状态迁移", () => {
    test("应该正确迁移状态到不同适配器", async () => {
      // 创建Fabric状态
      const fabricStateId = stateManager.createState("fabric", {
        src: "test.jpg",
      });
      ...
      // 迁移到Konva
      const konvaStateId = await stateManager.migrateState(
        fabricStateId,
        "konva"
      );
    ...
    test("应该在状态不存在时抛出错误", async () => {
      await expect(
        stateManager.migrateState("non-existent", "konva")
      ).rejects.toThrow("Source state not found: non-existent");
    });
  });
  ...
  describe("兼容性检查", () => {
    test("应该正确检查状态兼容性", () => {
      ...
      const compatibility = stateManager.checkStateCompatibility(
        stateId,
        "jimp"
      );
    ...
    test("应该正确识别兼容的状态", () => {
      ...
      state.filters = [{ type: "grayscale" }, { type: "blur" }];
