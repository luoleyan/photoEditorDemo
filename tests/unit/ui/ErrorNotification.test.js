...
describe('ErrorNotification', () => {
  ...
  describe('基本功能', () => {
    ...
    test('应该限制通知数量', () => {
      ...
      expect(wrapper.vm.visibleNotifications.length).toBeLessThanOrEqual(wrapper.vm.maxNotifications);
    });
  ...
  describe('操作指导功能', () => {
    ...
    test('应该正确推断操作类型', () => {
      ...
      expect(wrapper.vm.getOperationType(notification)).toBe('filter_application');
    });
  ...
  describe('故障排除功能', () => {
    ...
    test('应该正确推断问题类型', () => {
      expect(wrapper.vm.getIssueType({ type: 'network' })).toBe('network_issues');
      expect(wrapper.vm.getIssueType({ type: 'memory' })).toBe('performance_issues');
      expect(wrapper.vm.getIssueType({ type: 'adapter' })).toBe('feature_issues');
      expect(wrapper.vm.getIssueType({ type: 'ui' })).toBe('feature_issues');
      expect(wrapper.vm.getIssueType({ type: 'unknown' })).toBe('feature_issues');
    });
    ...
    test('应该显示故障排除步骤', () => {
      const mockTroubleshooting = {
        ...
        steps: [
          ...
          { step: '清理历史', description: '清除记录', action: 'clear_history' }
        ]
      };
  ...
  describe('网络连接检查', () => {
    ...
    test('应该检查网络连接不稳定', async () => {
      ...
      expect(wrapper.emitted('warning-message')[0][0]).toBe('⚠️ 网络连接不稳定');
    });
  });
