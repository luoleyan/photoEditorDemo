...
describe("拖拽功能测试", () => {
    ...
    it("应该能够拖拽性能监控器", () => {
      ...
      cy.get(".performance-monitor").then(($monitor) => {
        ...
        cy.get(".performance-monitor .drag-handle")
          .trigger("mousedown", {
            clientX: initialRect.x + 10,
            clientY: initialRect.y + 10,
          })
          .trigger("mousemove", {
            clientX: initialRect.x + 100,
            clientY: initialRect.y + 50,
          })
          .trigger("mouseup");
    ...
    it("小幅移动不应该触发拖拽", () => {
      ...
      cy.get(".performance-monitor").then(($monitor) => {
        ...
        cy.get(".performance-monitor .drag-handle")
          ...
          .trigger("mousemove", {
            clientX: initialRect.x + 12,
            clientY: initialRect.y + 12,
          }) // 只移动2像素
          .trigger("mouseup");
    ...
    it("超过阈值的移动应该触发拖拽", () => {
      ...
      cy.get(".performance-monitor").then(($monitor) => {
        ...
        cy.get(".performance-monitor .drag-handle")
          ...
          .trigger("mousemove", {
            clientX: initialRect.x + 20,
            clientY: initialRect.y + 20,
          }) // 移动10像素
          .trigger("mouseup");
  ...
  describe("边缘吸附功能", () => {
    it("拖拽到边缘应该自动吸附", () => {
      ...
      // 验证吸附状态
      cy.get(".performance-monitor").should("have.class", "snapped");

      // 验证触发区域存在
      cy.get(".snap-trigger-area").should("be.visible");
    });
    ...
    it("点击触发区域应该取消吸附", () => {
      ...
      // 验证吸附状态被取消
      cy.get(".performance-monitor").should("not.have.class", "snapped");
    });
  });
  ...
  it("应该支持触摸拖拽", () => {
    ...
    cy.get(".performance-monitor").then(($monitor) => {
      ...
      cy.get(".performance-monitor .drag-handle")
        .trigger("touchstart", {
          touches: [
            { clientX: initialRect.x + 10, clientY: initialRect.y + 10 },
          ],
        })
        .trigger("touchmove", {
          touches: [
            { clientX: initialRect.x + 100, clientY: initialRect.y + 50 },
          ],
        })
        .trigger("touchend");
