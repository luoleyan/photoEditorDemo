<template>
  <div class="brush-tool" :class="brushClasses">
    ...
    <div class="brush-toolbar" v-if="showToolbar">
      ...
      <div class="toolbar-section">
        ...
        <div class="brush-selector">
          <button
            v-for="brush in availableBrushes"
            :key="brush.type"
            class="brush-button"
            :class="{ active: selectedBrushType === brush.type }"
            @click="selectBrushType(brush.type)"
            :title="brush.name"
            :disabled="disabled"
          >
      ...
      <div class="toolbar-section">
        ...
        <div class="brush-opacity-control">
          ...
          <input
            ...
            max="100"
            step="1"
            @input="handleBrushOpacityChange"
            :disabled="disabled"
          />
    ...
    <div v-if="showAdvancedSettings" class="advanced-settings">
      ...
      <div class="setting-group">
        ...
        <input
          ...
          max="100"
          step="1"
          @input="handleBrushHardnessChange"
          :disabled="disabled"
        />
      ...
      <div class="setting-group">
        ...
        <input
          type="range"
          v-model.number="brushFlow"
          min="1"
          max="100"
          step="1"
          @input="handleBrushFlowChange"
          :disabled="disabled"
        />
      ...
      <div class="setting-group">
        ...
        <input
          ...
          max="100"
          step="1"
          @input="handleBrushSpacingChange"
          :disabled="disabled"
        />
      ...
      <div class="setting-group">
        ...
        <input
          type="range"
          v-model.number="brushSmoothing"
          min="0"
          max="100"
          step="1"
          @input="handleBrushSmoothingChange"
          :disabled="disabled"
        />
    ...
    <div class="brush-canvas-container" ref="canvasContainer">
      ...
      <!-- 光标预览 -->
      <div
        v-if="showCursorPreview && !disabled && !isDrawing"
  ...
  methods: {
    ...
    continueDrawing(x, y, pressure) {
      ...
      const smoothPressure =
        this.lastPressure + (pressure - this.lastPressure) * (1 - smoothing);
    ...
    drawPoint(context, x, y, pressure) {
      ...
      context.globalCompositeOperation = this.currentStroke.isEraser
        ? "destination-out"
        : this.getCanvasBlendMode(this.currentStroke.blendMode);
    ...
    drawLine(context, x1, y1, x2, y2, pressure1, pressure2) {
      ...
      const spacing = Math.max(
        1,
        this.currentStroke.size * this.currentStroke.spacing
      );
    ...
    drawStroke(context, stroke) {
      if (!context || !stroke || !stroke.points || stroke.points.length === 0)
        return;
      ...
      context.globalCompositeOperation = stroke.isEraser
        ? "destination-out"
        : this.getCanvasBlendMode(stroke.blendMode);
    ...
    updateBrushPreview() {
      ...
      // 清空预览画布
      this.brushPreviewCtx.clearRect(
        0,
        0,
     ...
     */
    getCurrentBrushName() {
      const brush = this.availableBrushes.find(
        (b) => b.type === this.selectedBrushType
      );
    ...
    async _enableAdapterDrawingMode() {
      ...
      try {
        ...
        switch (this.adapterType) {
          ...
          default:
            console.warn(
              `Drawing mode not supported for adapter type: ${this.adapterType}`
            );
        }
      } catch (error) {
        console.error("Failed to enable adapter drawing mode:", error);
    ...
    async _syncStrokeToAdapter(stroke) {
      ...
      try {
        ...
        switch (this.adapterType) {
          ...
          default:
            console.warn(
              `Stroke sync not supported for adapter type: ${this.adapterType}`
            );
        }
      } catch (error) {
        console.error("Failed to sync stroke to adapter:", error);
