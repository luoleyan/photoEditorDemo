...
class TuiAdapter extends BaseImageEditorAdapter {
  ...
  async _doInitialize() {
    return this._safeExecute(async () => {
      ...
      try {
        ...
        console.error('TUI ImageEditor constructor failed:', constructorError);
        ...
        throw new Error(`Failed to create TUI ImageEditor instance: ${constructorError.message}`);
      }
    ...
    return this._safeExecute(async () => {
      ...
      return new Promise((resolve, reject) => {
        this.editor.loadImageFromURL(imageData.src, 'image')
          .then(() => {
            try {
              console.log('TuiAdapter: Image loaded successfully, getting dimensions...');
              ...
              setTimeout(() => {
                try {
                  ...
                  console.log('TuiAdapter: Image data saved:', this.originalImageData);
                  ...
                  console.error('TuiAdapter: Failed to get image dimensions:', dimensionError);
                  reject(new Error(`Failed to get image dimensions: ${dimensionError.message}`));
                }
              }, 200); // 200ms delay to ensure image is fully loaded

            } catch (error) {
              console.error('TuiAdapter: Error in image load success handler:', error);
              reject(error);
            }
          })
          .catch((error) => {
            console.error('TuiAdapter: Failed to load image from URL:', error);
  ...
  async _doSetBrightness(value) {
    return this._safeExecute(async () => {
      ...
      if (typeof this.editor.applyFilter !== 'function') {
        throw new Error('applyFilter method not available in TUI Image Editor');
      }

      console.log('TuiAdapter: Applying brightness filter with value:', value);
      ...
      try {
        ...
        console.error('TuiAdapter: Failed to apply brightness filter:', filterError);
        throw new Error(`Failed to apply brightness filter: ${filterError.message}`);
      }
    }, 'setBrightness', { value });
  }
  ...
  async _doSetContrast(value) {
    return this._safeExecute(async () => {
      ...
      try {
        ...
        console.error('TuiAdapter: Failed to apply contrast filter:', filterError);
        throw new Error(`Failed to apply contrast filter: ${filterError.message}`);
      }
    }, 'setContrast', { value });
  }
  ...
  async _doApplyFilter(filterType, options = {}) {
    return this._safeExecute(async () => {
      ...
      console.log('TuiAdapter: Applying filter:', filterType, 'with options:', options);
      ...
      try {
        ...
        console.log('TuiAdapter: Filter applied successfully:', tuiFilterType);
        this._updatePerformanceMetrics();
      } catch (filterError) {
        console.error('TuiAdapter: Filter application failed:', filterError);
        throw new Error(`Failed to apply ${filterType} filter: ${filterError.message}`);
      }
  ...
  _getImageDimensions() {
    ...
    // 方法4: 使用默认尺寸
    console.warn('All dimension detection methods failed, using default dimensions');
  ...
  async _safeApplyFilter(filterType, options = {}) {
    ...
    try {
      ...
      console.log(`Filter ${filterType} applied successfully with options:`, options);
      return;
    } catch (error) {
    ...
    try {
      ...
      if (filterType === 'brightness' && options.brightness !== undefined) {
        ...
        console.log(`Brightness filter applied with value: ${options.brightness}`);
        return;
      }
      ...
      console.warn(`Alternative parameter format failed for ${filterType}:`, error);
    }

    // 方法3: 如果所有方法都失败，抛出错误
    throw new Error(`Failed to apply ${filterType} filter with all available methods`);
  }
  ...
  _performMemoryCleanup() {
    try {
      ...
      if (!this.stateHistory) {
        console.log('TuiAdapter: Adapter already destroyed, skipping memory cleanup');
        return;
      }

      // 安全清理状态历史
      if (this.stateHistory && typeof this.stateHistory.size === 'number' && this.stateHistory.size > 20) {
        try {
          const entries = Array.from(this.stateHistory.entries());
          ...
          console.log(`TuiAdapter: Cleaned up ${toDelete.length} old state entries`);
        } catch (error) {
      ...
      if (window.gc) {
        try {
          ...
          console.warn('TuiAdapter: Failed to trigger garbage collection:', error);
        }
      }
